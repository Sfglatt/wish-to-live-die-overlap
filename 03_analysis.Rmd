...
write.csv(tab,
  file = paste0(
    "03_output/WTL_WTD_cross_table_",
...
# Make standardized versions for direct magnitude comparability
Baseline_data <- Baseline_data %>%
  mutate(
    q01_ssi_z = scale(q01_ssi),
    q02_ssi_z = scale(q02_ssi)
  )

# WTL without WTD (both standardized)
Baseline_data$WTLz_unique <- resid(lm(q01_ssi_z ~ q02_ssi_z, data = Baseline_data))

# WTD without WTL (both standardized)
Baseline_data$WTDz_unique <- resid(lm(q02_ssi_z ~ q01_ssi_z, data = Baseline_data))


# Summary
describe(Baseline_data[, c(
  "WTL_unique", "WTL_unique_rev",
  "WTD_unique", "WTD_unique_rev",
  "WTLz_unique", "WTDz_unique"
)])
```
...
# equivalent to prcomp(Baseline_data[, c("q01_ssi_z", "q02_ssi_z")], scale. = FALSE)
...
# Because of non–positive definite information matrix, constrain to equal loadings
model_cfa_equal <- "
...
tri <- Baseline_data %>%
  dplyr::transmute(
    `WTL Unique` = abs(WTLz_unique),
    `Shared Component` = abs(WTL_WTD_shared),
    `WTD Unique` = abs(WTDz_unique)
  )
...
Baseline_data$Dominant_signed <- with(
  Baseline_data,
  ifelse(
    Dominant_component == "WTL Unique", WTLz_unique,
    ifelse(
      Dominant_component == "WTD Unique", WTDz_unique,
      WTL_WTD_shared
    )
  )
)
...
Baseline_data$WTL_unique_abs <- abs(Baseline_data$WTLz_unique)
Baseline_data$WTD_unique_abs <- abs(Baseline_data$WTDz_unique)
...
describe(Baseline_data[, c("WTL_unique_abs", "WTD_unique_abs", "Unique_max_abs", "WTL_WTD_shared_abs")])
...
# Overall model R2/significance for WTL and WTD (raw)
```{r regr concurrent}
...
# Decompose R2 with commonality analysis 
```{r commonality}
...
# Unique and shared variation correlations 
...
Corr_vars <- Baseline_data %>%
  dplyr::select(
    q01_ssi, # Raw WTL
    q02_ssi, # Raw WTD
    WTLz_unique, # WTL without WTD
    WTDz_unique, # WTD without WTL
...
# WTL residuals correlation (linear vs rank-based)
round(cor(Baseline_data$WTL_unique,
  Baseline_data$WTL_unique_rank,
  use = "p"
), 5) # 0.980

# WTD residuals correlation (linear vs rank-based)
round(cor(Baseline_data$WTD_unique,
  Baseline_data$WTD_unique_rank,
  use = "p"
), 5) # 0.992

# Shared correlation (raw PCA vs rank-based PCA)
round(cor(Baseline_data$WTL_WTD_shared,
  Baseline_data$WTL_WTD_shared_rank,
  use = "p"
), 5) # 0.994
```
...
cor_results <- lapply(outcomes, function(out) {
  ...
  pearson_WTL_resid <- cor(Baseline_data$WTLz_unique, Baseline_data[[out]], use = "pairwise.complete.obs", method = "pearson")
  pearson_WTD_resid <- cor(Baseline_data$WTDz_unique, Baseline_data[[out]], use = "pairwise.complete.obs", method = "pearson")
  ...
  polyserial_WTL_resid <- polycor::polyserial(Baseline_data[[out]], Baseline_data$WTLz_unique, ML = TRUE)
  polyserial_WTD_resid <- polycor::polyserial(Baseline_data[[out]], Baseline_data$WTDz_unique, ML = TRUE)
...
# Standardize fir each timepoint
Temporal_data <- Temporal_data %>%
  group_by(month) %>%
  mutate(
    q01_ssi_z = as.numeric(scale(q01_ssi)),
    q02_ssi_z = as.numeric(scale(q02_ssi))
  ) %>%
  ungroup()

# Residualize for each timepoint
Temporal_data <- Temporal_data %>%
  group_by(month) %>%
  mutate(
    WTLz_unique = resid(lm(q01_ssi_z ~ q02_ssi_z, na.action = na.exclude)),
    WTDz_unique = resid(lm(q02_ssi_z ~ q01_ssi_z, na.action = na.exclude))
  ) %>%
  ungroup()

# Extract shared component for each timepoint
Temporal_data <- Temporal_data %>%
  group_by(month) %>%
  mutate(
    shared = {
      ...
      shared_vec <- rep(NA, n()) # Deal with missing without deleting
...
baseline_check <- Temporal_data %>%
  filter(month == 0) %>%
  select(id, WTLz_unique, WTDz_unique, shared) %>%
  left_join(
    Baseline_data %>%
      select(id, WTLz_unique, WTDz_unique, WTL_WTD_shared),
    by = "id",
...
baseline_check %>%
  summarise(
    cor_WTL = cor(WTLz_unique_temporal, WTLz_unique_baseline, use = "pairwise.complete.obs"),
    cor_WTD = cor(WTDz_unique_temporal, WTDz_unique_baseline, use = "pairwise.complete.obs"),
    cor_shared = cor(shared, WTL_WTD_shared, use = "pairwise.complete.obs")
  ) # Good

# Another sanity check to make sure NA logic worked
# Isolate the longitudinal dataset to month 3 and remove people missing raw WTL/WTD
month3_check <- Temporal_data %>%
  filter(month == 3) %>%
  select(id, q01_ssi_z, q02_ssi_z, WTLz_unique, WTDz_unique, shared, q01_ssi, q02_ssi) %>%
  filter(!is.na(q01_ssi) & !is.na(q02_ssi))

# Recalculate residuals and PCA for month 3 only
# WTL without WTD
model_WTL_on_WTD_m3 <- lm(q01_ssi_z ~ q02_ssi_z, data = month3_check)
month3_check$WTL_unique_check <- residuals(model_WTL_on_WTD_m3)

# WTD without WTL
model_WTD_on_WTL_m3 <- lm(q02_ssi_z ~ q01_ssi_z, data = month3_check)
month3_check$WTD_unique_check <- residuals(model_WTD_on_WTL_m3)

# PCA
pc_m3 <- prcomp(month3_check[, c("q01_ssi_z", "q02_ssi_z")], scale. = FALSE)
month3_check$shared_check <- pc_m3$x[, 1]

# Compare
month3_check %>%
  summarise(
    cor_WTL = cor(WTLz_unique, WTL_unique_check, use = "pairwise.complete.obs"),
    cor_WTD = cor(WTDz_unique, WTD_unique_check, use = "pairwise.complete.obs"),
    cor_shared = cor(shared, shared_check, use = "pairwise.complete.obs")
  ) # Good
```
...
n_distinct(Temporal_data$id) # 192
nrow(Temporal_data) # 586
nrow(Temporal_data %>% filter(!is.na(shared))) # 537 complete obs

# Shared component
model_shared <- lmer(shared ~ 1 + (1 | id), data = Temporal_data)

# WTL_unique
model_WTL_unique <- lmer(WTLz_unique ~ 1 + (1 | id), data = Temporal_data)

# WTD_unique
model_WTD_unique <- lmer(WTDz_unique ~ 1 + (1 | id), data = Temporal_data)

# WTD raw
model_WTD_raw <- lmer(q02_ssi ~ 1 + (1 | id), data = Temporal_data)

# WTL raw
model_WTL_raw <- lmer(q01_ssi ~ 1 + (1 | id), data = Temporal_data)
...
sjPlot::tab_model(
  model_shared, model_WTD_unique, model_WTL_unique, model_WTL_raw, model_WTD_raw
)
```

# Save baseline dataset with new variables
- residualized WTD/WTL (raw)
- residualized WTD/WTL (z-transformed)
- rank-transformed WTL/WTD and those residuals
- the shared PCA component (raw and from rank-transformed)
- absolute values of residualized WTD/WTL (z-transformed)
- absolute value of shared PCA component (z-transformed)
- dominant component of absolute values

# Save longitudinal dataset with new variables
- z-transformed WTD/WTL (each timepoint)
