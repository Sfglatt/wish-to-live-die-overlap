...
Baseline_data <- read.csv("03_output/Baseline_data_vars_2025-12-28.csv")
```
...
plot_data <- Baseline_data %>%
  ...
  dplyr::select(WTLz_unique, WTDz_unique, WTL_WTD_shared) %>%
  rename(
    `WTL Unique` = WTLz_unique,
    `WTD Unique` = WTDz_unique,
...
# Data for MLM figures
```{r data temp}
Temporal_data <- read.csv("03_output/Temporal_data_vars_2025-12-28.csv")
```

# Visualize random effects
```{r ranef vis}
# Models to extract effects from
...
# Extract random intercepts for people
re_shared <- ranef(model_shared, condVar = TRUE)$id %>%
  tibble::rownames_to_column("id") %>%
  rename(re_shared = `(Intercept)`)

re_WTL <- ranef(model_WTL_unique, condVar = TRUE)$id %>%
  tibble::rownames_to_column("id") %>%
  rename(re_WTL = `(Intercept)`)

re_WTD <- ranef(model_WTD_unique, condVar = TRUE)$id %>%
  tibble::rownames_to_column("id") %>%
  rename(re_WTD = `(Intercept)`)

# Merge
re_all <- re_shared %>%
  left_join(re_WTL, by = "id") %>%
  left_join(re_WTD, by = "id")

# Reshape data to long format for vis
re_long <- re_all %>%
  pivot_longer(
    cols = c(re_shared, re_WTL, re_WTD),
    names_to = "component",
    values_to = "random_intercept"
  ) %>%
  mutate(component = recode(component,
    re_shared = "Shared Component",
    re_WTL = "Unique WTL",
    re_WTD = "Unique WTD"
  ))

# Faceted density plot
ggplot(re_long, aes(x = random_intercept, fill = component)) +
  geom_density(alpha = 0.4) +
  scale_fill_manual(values = c("mediumpurple", "#66CCFF", "#FF66CC")) +
  labs(
    x = "Random Intercept",
    y = "Density",
    fill = "Component"
